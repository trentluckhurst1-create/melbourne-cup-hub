...
function coverageSnapshot(){
  ...
  const pfrIdentity=(cupData?.horses||[]).filter(h=>typeof privateTfMatched==='function'&&privateTfMatched(h.horse)).length;
  const marketRows=typeof marketUniverse==='function'?marketUniverse():[];
  const marketCoverage=intersectCount(marketRows.filter(x=>x?.last).map(x=>x.horse));
  const marketGaps=Math.max(0,total-marketCoverage);
  return {total,verifiedBases,profiles,weights,projected,trainerCoverage,qualifiedNominees,formCoverage,fullForm,formGaps,formFlags,pfrIdentity,marketCoverage,marketGaps};
}
...
function queueColumn(title,names,kind,detailFn=null){return `<div class="queue-col"><div class="queue-title">${title}</div>${names.length?names.map((name,i)=>`<button class="queue-item" onclick="${horseByName(name)?`openHorse('${name.replace(/'/g,"\\'")}')`:'void 0'}"><span>${String(i+1).padStart(2,'0')}</span><strong>${name}</strong><em>${detailFn?detailFn(name):kind}</em></button>`).join(''):'<div class="queue-empty">Complete</div>'}</div>`;}

function coveragePanel(){
  const c=coverageSnapshot();
  const q=researchQueue();
  return `<section class="section-block"><div class="section-header"><div><div class="kicker">Research Coverage Audit</div><h2>101-Horse Intelligence Completion</h2><div class="section-copy">Every percentage is calculated against the official nomination file. A horse remains in the universe even when evidence is missing; only verified or dated evidence counts toward completion.</div></div></div>
    <div class="coverage-grid">
      ...
      ${coverageBar('Private PFR identity matched',c.pfrIdentity,c.total,'Identity layer only. Private numerical ratings remain local and separate from public PFR.')}
      ${coverageBar('Dated market evidence',c.marketCoverage,c.total,`${c.marketGaps} nominees remain visible as MARKET GAP; an empty market shell does not count as coverage.`)}
      ...
      ${coverageBar('Projected-field assessment',c.projected,c.total,'Top-24 is a current signal only; all other nominees remain in the permanent 101-horse universe.')}
    </div>
