...
function leadupStatusTag(status){
  ...
  if(status==='Due to race') return tag(status,'green');
...
function leadupsView(){
  ...
  const pending=events.filter(x=>x.status==='Due to race').length;
  ...
  <section class="metric-grid">
    ...
    ${metric('Pending Starts',pending,'Sourced future engagement')}
    ${metric('Latest','13 Sep','Goodie Two Shoes engagement')}
  </section>
