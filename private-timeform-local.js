...
function bindPrivateTfControls(){
  ...
  const clear=document.getElementById('private-tf-clear'); if(clear)clear.onclick=()=>{privateTimeformSession=null;closePrivateTfPanel();render(currentView);};
}
...
function tfWindowStats(horse){
  const rec=tfHorseRec(horse); const vals=tfRuns(rec).map(tfRatingValue).filter(Number.isFinite);
  if(!vals.length)return {count:0,peak:null,avg:null,last:null};
  return {count:vals.length,peak:Math.max(...vals),avg:(vals.reduce((a,b)=>a+b,0)/vals.length).toFixed(1),last:vals[0]};
}
...
function openPrivateTfPanel(horse){
  if(!privateTimeformSession){alert('Load your private Timeform JSON first.');return;}
...
formGuideView=function(){
  ...
  return privateTfControls()+`<div class="section-header"><div><div class="kicker">Private Timeform + Public Form</div><h2>Melbourne Cup Form Guide</h2><div class="section-copy">Click any horse name or gold TFR to open the complete private rating panel for that runner.</div></div></div><div class="horse-grid private-form-grid">${cards}</div>`;
};
