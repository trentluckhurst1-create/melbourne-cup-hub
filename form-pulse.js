...
function formPulseFreshness(x){
  ...
  const ms=Date.now()-new Date(`${d}T00:00:00+10:00`).getTime();
...
function formPulseMarket(x){
  if(!x.market)return '—';
  const pct=x.market.material?Math.abs(x.market.impliedMovePct||0).toFixed(0):null;
  const move=x.market.material?`<small class="${x.market.direction==='SHORTENED'?'fp-market-up':'fp-market-down'}">${x.market.direction==='SHORTENED'?'▲':'▼'} ${pct}% implied</small>`:`<small>${x.market.last.bookmaker}</small>`;
  return `$${x.market.last.odds.toFixed(2)}${move}`;
}
function formPulseFiltered(){
  return formPulseRows().filter(x=>{
    ...
    if(formPulseFilter==='market')return !!x.market?.material;
...
function formPulseControls(){
  const opts=[['all','Projected 24'],['improving','Improving'],['stayers','2800m+'],['fresh','Ran ≤21d'],['market','Market movers'],['gaps','Form gaps']];
...
function formPulseLeaders(all){
  ...
  const market=[...all].filter(x=>x.market?.material).sort((a,b)=>Math.abs(b.market.impliedMovePct)-Math.abs(a.market.impliedMovePct))[0]||null;
  return {current,staying,improver,fresh,lens,market};
}
function formPulseLeaderStrip(all){
  ...
  const marketValue=l.market?`${l.market.market.direction==='SHORTENED'?'+':'-'}${Math.abs(l.market.market.impliedMovePct).toFixed(0)}%`:'—';
  return `<div class="fp-leaders">
    ...
    ${card('MARKET PRESSURE',l.market,marketValue,l.market?`${l.market.market.previousSameBook?`$${l.market.market.previousSameBook.odds.toFixed(0)} → `:''}$${l.market.market.last.odds.toFixed(0)} · ${l.market.market.last.bookmaker}`:'No material same-book move')}
...
function formPulsePanel(){
  ...
  const movers=all.filter(x=>x.market?.material).length;
  const best=[...all].filter(x=>x.lens).sort((a,b)=>b.lens.score-a.lens.score)[0]||null;
  return `<section class="panel cc-panel fp-panel"><div class="cc-panel-head"><div><span class="cc-label">FORM + CLASS + STAMINA + MARKET</span><h3>Projected 24 · Form Pulse</h3><div class="panel-sub">Recent public performance ratings, trajectory, staying peak, handicap and same-book market pressure. Market moves influence the profile label but do not alter PFR itself.</div></div><button class="mini-button" onclick="openView('ratings')">Full ratings →</button></div>
    <section class="fp-metrics"><div><span>RATED</span><strong>${rated}/24</strong></div><div><span>FORM COMPLETE</span><strong>${complete}/24</strong></div><div><span>IMPROVING</span><strong>${improving}</strong></div><div><span>2800M+ RATED</span><strong>${stayers}</strong></div><div><span>MARKET MOVERS</span><strong>${movers}</strong></div><div><span>CUP LENS</span><strong>${best?best.p.horse:'—'}</strong><em>${best?best.lens.score.toFixed(1):'—'}</em></div></section>
