...
function dossierRunRows(name){const runs=dossierRatedRuns(name);if(!runs.length)return '<tr><td colspan="10" class="order-empty">No verified public race starts loaded yet.</td></tr>';return runs.map(r=>`<tr><td>${r.date||'—'}</td><td>${r.track||'—'}</td><td class="wrap-cell">${r.race||'—'}</td><td>${r.distanceM?`${r.distanceM}m`:'—'}</td><td>${r.going||'—'}</td><td>${r.weightKg?`${r.weightKg}kg`:r.weightCarried||r.weight||'—'}</td><td class="dossier-run-result">${r.finish||'—'}</td><td>${r.margin??'—'}</td><td><strong>${Number.isFinite(r.publicRating)?r.publicRating.toFixed(1):'—'}</strong></td><td>${r.performanceRating??'—'}</td></tr>`).join('');}
...
horseDetailView=function(name){
  ...
  <section class="panel dossier-section"><div class="panel-head"><div><h3>Latest Public Form & Ratings</h3><div class="panel-sub">Each verified race start now carries a transparent Hub Public Form Rating. The TF column remains empty unless a genuine private Timeform run rating exists.</div></div><span class="tag ${formComplete?'green':'gold'}">${formComplete?formState.state.replace(/_/g,' '):`${runs.length}/8 LOADED`}</span></div><div class="table-wrap"><table class="data-table"><thead><tr><th>Date</th><th>Track</th><th>Race</th><th>Dist.</th><th>Going</th><th>Weight</th><th>Finish</th><th>Margin</th><th>PFR</th><th>TF</th></tr></thead><tbody>${dossierRunRows(name)}</tbody></table></div></section>
