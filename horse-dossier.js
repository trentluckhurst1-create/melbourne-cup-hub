...
horseDetailView=function(name){
  ...
  const base=dossierBase(name);const proj=dossierProjected(name);const w=dossierWeight(name);const official=dossierOfficialWeight(name);const lead=dossierLeadup(name);const runs=dossierForm(name);const formState=dossierFormState(name);const tf=dossierTf(name);const qual=dossierQual(name);const stamina=dossierStamina(name);const ready=dossierReady(name);const market=dossierMarketRec(name);
  ...
  const badges=[tag(h.status||'Nominated'),qual==='Golden Ticket'?tag('Golden Ticket','green'):null,proj?tag(`Projected #${proj.rank}`,proj.rank<=19?'green':'gold'):null,tf?tag('TF identity matched','green'):tag('TF unresolved','red'),Number.isFinite(weightDelta)&&Math.abs(weightDelta)>=1.5?tag('Weight shock','red'):null].filter(Boolean).join('');
