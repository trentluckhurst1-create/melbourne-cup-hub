...
(function(){
 ...
 const raceKey=r=>`${dateOf(r)}|${norm(r?.race||r?.raceName)}|${norm(r?.track||r?.venue)}`;
 const sameStart=(a,b)=>{
  const da=dateOf(a),db=dateOf(b);if(!da||da!==db)return false;
  const ar=norm(a?.race||a?.raceName),br=norm(b?.race||b?.raceName),at=norm(a?.track||a?.venue),bt=norm(b?.track||b?.venue);
  if(ar&&br&&at&&bt)return ar===br&&at===bt;
 ...
 function richestRuns(name){
  ...
  const rows=[];for(const r of pools){if(!r)continue;const d=dateOf(r);if(!d)continue;let i=rows.findIndex(x=>raceKey(x)===raceKey(r));if(i<0)i=rows.findIndex(x=>sameStart(x,r));if(i<0)rows.push({...r});else rows[i]=mergeRich(rows[i],r);}
