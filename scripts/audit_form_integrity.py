...
def main():
 ...
 for nk,name in ni.items():
  ...
  for i,r in enumerate(runs):
   ...
   rk=(norm(r.get('race')),norm(r.get('track')))
   if rk[0] and rk[1]:
    if rk in seen_race and seen_race[rk]!=ds: issues.append({'severity':'WARNING','code':'REPEATED_RACE_TRACK_IDENTITY','run':i,'otherDate':seen_race[rk],'date':ds,'race':r.get('race'),'track':r.get('track')})
 ...
 if any(x['severity']=='CRITICAL' for x in global_issues): critical+=1
 out={'snapshotDate':'2026-09-17','universe':len(names),'canonicalFile':str(FORM.relative_to(ROOT)).replace('\\','/'),'policy':'Canonical recent form is an eight-run-or-complete-career window only. It is never a full-career statistics source. One actual start per horse/date; no trials, jump-outs, non-starters, future runs, impossible finish/field relationships, or non-nominee identities.','summary':{'pass':sum(r['status']=='PASS' for r in rows),'warning':warning,'criticalHorses':sum(r['status']=='CRITICAL' for r in rows),'globalCritical':any(x['severity']=='CRITICAL' for x in global_issues)},'globalIssues':global_issues,'rows':rows}
