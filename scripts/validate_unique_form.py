...
def main():
    ...
    out={'snapshotDate':'2026-09-15','type':'strict-unique-public-form','targetRunsPerHorse':8,'dedupeRule':'one actual start per horse per calendar date; trials/jump-outs/non-starters excluded','sourceFiles':files,'horses':{}}
    rows=[]
    for h in noms['horses']:
        ...
        runs=allruns[:8]
        # A career-complete flag is only needed if fewer than 8 unique actual starts exist.
    ...
    print(json.dumps({'complete':complete,'careerComplete':career,'resolved':complete+career,'partial':partial,'zero':zero}))
