...
function qualificationFor(name){
  const q=(qualificationData?.qualified||qualificationData?.horses||[]);
  if(Array.isArray(q)) return q.find(x=>x.horse===name)||null;
  return null;
}

function internationalRaidersView(){
  const horses=(cupData?.horses||[]).filter(h=>h.trainingRegion==='International');
...
render=function(view='dashboard'){
  if(view==='international'){
    document.getElementById('page-title').textContent='International Raiders';
    const root=document.getElementById('app-content');
    const pending=[];
    if(!trainerBaseData) pending.push(loadTrainerBases());
